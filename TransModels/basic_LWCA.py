...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, y, mask=None):
        ...
        relative_position_bias = self.relative_position_bias_table[self.relative_position_index.view(-1)].view(
            ...
            self.window_size[0] * self.window_size[1], -1)  # Wh*Ww*Wt,Wh*Ww*Wt,nH
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, y, mask_matrix):
        ...
        if min(self.shift_size) > 0:
            ...
            shifted_y = torch.roll(y, shifts=(-self.shift_size[0], -self.shift_size[1]), dims=(1, 2))
        ...
        x_windows = x_windows.view(-1, self.window_size[0] * self.window_size[1], C)  # nW*B, window_size*window_size*window_size, C
        y_windows = window_partition(shifted_y, self.window_size)  # nW*B, window_size, window_size, window_size, C
        y_windows = y_windows.view(-1, self.window_size[0] * self.window_size[1], C)  # nW*B, window_size*window_size*window_size, C
        # W-MSA/SW-MSA
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x, y, H, W):
        ...
        # print('H W:', H, W)
        # print('windows_size:', self.window_size[0], self.window_size[1])
...
class SwinTransformer(nn.Module):
    ...
    def forward(self, x, y):
        ...
        for i in range(1):  # num_layers  = len(depths)  = 4
            layer = self.layers[i]
            x_out, H, W, x, Wh, Ww = layer(x, y, Wh, Ww)
            # print('ok')
